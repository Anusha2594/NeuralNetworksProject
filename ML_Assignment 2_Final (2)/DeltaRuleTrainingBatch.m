function [w, iterations, e,ePerIteration,predictedW]=DeltaRuleTrainingBatch(Data, Target, eta, error, epochs,isDecay,decayingRate,isAdaptive, positiveRate,treshold)
...
if rd ~= rt
    ...
    while e >= error &&  iterations < epochs
        ...
        if isDecay == 1
            eta = eta*decayingRate;
        end
        if isAdaptive == 1 && iterations > 1
            errorDif = e - previousError;
            if errorDif >= treshold
                w = prevW;
                eta = eta * decayingRate;
            else
                eta = prevETA;
                eta = eta * positiveRate;
            end   
        end
    end
end