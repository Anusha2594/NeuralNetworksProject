...
positiveRate = 1.02;
treshold = 1.03;
%Delta Training Rule batch start with adaptive rates
...
m = ((-1)*wBatch(1))/wBatch(2);
yIntercept = ((-1)*wBatch(3))/wBatch(2);
x = -10:10;
a = plot(x, ((m*x)+yIntercept));
hold on;
...
hold on;
legend({'5 iterations','10 iterations','50 iterations','100 iterations','final prediction','target'},'Location','southwest')
for i=1:rd
    if T(i) == 1
        s = 'r*';
    else
